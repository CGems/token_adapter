  ...
  module Gemther
    ...
    class Gem < TokenAdapter::Base
      ...
      def initialize(config)
        super(config)
        init_provider(TokenAdapter::Gemther.rpc_endpoint || config[:rpc])
        token_decimals = 18
      end
  
      # 获取交易所的地址上币
      def getbalance(account = nil)
        account ||= (config[:exchange_address] || TokenAdapter::Gemther.exchange_address)
        get_balance(account)
      end
  
      def getnewaddress(index, passphrase)
        xpub = TokenAdapter::Gemther.xpub || config[:xpub]
      ...
      def sendtoaddress(address, amount)
        gas_limit = config[:transfer_gas_limit] || TokenAdapter::Gemther.transfer_gas_limit
        gas_price = config[:transfer_gas_price] || TokenAdapter::Gemther.transfer_gas_price
      ...
      def wallet_collect(wallet_address, token_address, amount)
        ...
        gas_limit = config[:collect_gas_limit] || TokenAdapter::Gemther.collect_gas_limit
        gas_price = config[:collect_gas_price] || TokenAdapter::Gemther.collect_gas_price
      ...
      def from
        exchange_address_priv = config[:exchange_address_priv] || TokenAdapter::Gemther.exchange_address_priv
        exchange_address = config[:exchange_address] || TokenAdapter::Gemther.exchange_address
        exchange_address_passphrase = config[:exchange_address_passphrase] || TokenAdapter::Gemther.exchange_address_passphrase

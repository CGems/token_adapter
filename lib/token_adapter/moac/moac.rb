module TokenAdapter
  module Moac
    ...
    class Moac < TokenAdapter::Base
      include Provider::Rpc
      attr_accessor :token_decimals

      def initialize(config)
        super(config)
        init_provider(TokenAdapter::Moac.rpc_endpoint || config[:rpc])
        token_decimals = 18
      end

      # 获取交易所的地址上币
      def getbalance(account = nil)
        account ||= (config[:exchange_address] || TokenAdapter::Moac.exchange_address)
        get_balance(account)
      end

      def getnewaddress(index, passphrase)
        xpub = TokenAdapter::Moac.xpub || config[:xpub]
      ...
      def sendtoaddress(address, amount)
        gas_limit = config[:transfer_gas_limit] || TokenAdapter::Moac.transfer_gas_limit
        gas_price = config[:transfer_gas_price] || TokenAdapter::Moac.transfer_gas_price
      ...
      def wallet_collect(wallet_address, token_address, amount)
        function_signature = '6ea056a9' # Moac::Function.calc_id('sweep(address,uint256)')
        ...
        gas_limit = config[:collect_gas_limit] || TokenAdapter::Moac.collect_gas_limit
        gas_price = config[:collect_gas_price] || TokenAdapter::Moac.collect_gas_price
      ...
      def from
        exchange_address_priv = config[:exchange_address_priv] || TokenAdapter::Moac.exchange_address_priv
        exchange_address = config[:exchange_address] || TokenAdapter::Moac.exchange_address
        exchange_address_passphrase = config[:exchange_address_passphrase] || TokenAdapter::Moac.exchange_address_passphrase

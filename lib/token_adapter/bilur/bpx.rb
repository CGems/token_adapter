  ...
  module Bilur
    ...
    class Bpx < TokenAdapter::Base
      ...
      def wallet_collect(wallet_address, token_address, amount)
        ...
        gas_limit = config[:collect_gas_limit] || TokenAdapter::Bilur.collect_gas_limit
        gas_price = config[:collect_gas_price] || TokenAdapter::Bilur.collect_gas_price
      ...
      def from
        exchange_address_priv = config[:exchange_address_priv] || TokenAdapter::Bilur.exchange_address_priv
        exchange_address = config[:exchange_address] || TokenAdapter::Bilur.exchange_address
        exchange_address_passphrase = config[:exchange_address_passphrase] || TokenAdapter::Bilur.exchange_address_passphrase
